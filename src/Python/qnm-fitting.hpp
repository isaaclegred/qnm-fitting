...
/// Re{\psi_n(t)}, Im{\psi_1(t)}, ...  Im{\psi_n(t)}]\f$ params are \f$[x_1, ...x_n,
/// y_1, ... y_n, \omega_1, ... \omega_n, \Gamma_1, \Gamma_n]\f$, where \f$x_i  = A_i
/// cos(\phi_i)\f$, where \f$A_1\f$ is the real amplitude, and \f$\phi_1\f$ is the phase
/// \f$y_i = A_i sin(\phi_i)\f$, this stems from the complex vector with \f$N\f$ entries,
/// and converting it into a vector of \f$2N\f$ real numbers.  The conversion is
/// \f$c_n\f$, complex amplitude is \f$ c_n  = A_n e^{i \phi_n}\f$ and \f$\Omega_n\f$ a
/// complex frequency is \f$\Omega_n =  \omega_n + i \Gamma_n\f$
/// Breaking the expression down into a vector of real and imaginary parts
/// \f$c_n e^{i \Omega_n t} \sim  e^{-\Gamma_n t}[x_n cos \omega_n t - y_n sin \omega_n
/// t , x_n sin \omega_n t + y_n cos \omega_n t]\f$
...
/// \f$\Omega_j,\, \omega_j\f$ and \f$\Gamma_j)\f$, it will only be nonzero on the
/// diagonals of the N \times N sub-block matrices , The i ``rotates" the
/// real  part into the imaginary part, and the imaginary part into the
/// negative real part
/// derivative of with respect to \f$\Gamma_j\f$ \f$e^{-\Gamma_j t} \cos \omega_j t = -t e^{-\Gamma_j t}
/// \cos \omega_j t\f$ if real or \f$e^{-\Gamma_j t} sin \omega_j t  = -t e^{-Gamma_j t}
/// sin \omega_j t\f$
/// derivative of with respect to \f$\omega_j\f$,  \f$e^{-\Gamma_j t} \cos \omega_j t = -t e^{-\Gamma_j t}
/// \sin \omega_j t \f$
/// if real or \f$ e^{-\Gamma_j t} \sin \omega_j t = t e^{-\Gamma_j t} \cos \omega_j t \f$ if
/// imaginary
struct fitting_qnm {
  double end_time;
  ...
  void operator()(const Doub t, VecDoub_I &params, VecDoub_O &f_vals,
    ...
    for (size_t j = 0; j < num_modes; ++j) {
      ...
      // In this case the derivatives with respect to omega_j really matter, and the real
      // functions pick up an extra sign flip
