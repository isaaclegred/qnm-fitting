...
#include "test_Fitsep.h"

int main(int argc,char *argv[])  
{
	bool debug=false;
	Int i,j,N=100,MA=4,mfit=MA,P=MA/2,pfit=P;
	Doub aa[]={2.0,3.0,5.0,3.0};
	Doub cc[]={5.0,2.0};
//	Doub gguess[]={4.5,2.2,2.8,2.5,4.9,2.8};
	Doub gguess[]={2.2,2.8,4.9,2.8};
	Doub SPREAD=0.01;
	VecDoub x(N),y(N),sig(N);
	VecDoub a(MA,aa),guess(MA,gguess),c(P,cc);
	...
	Sep_marquardt myfit(x,y,sig,guess,fgauss_sep,P);
