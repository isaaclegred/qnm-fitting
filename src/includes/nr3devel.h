...
template <class T>
NRvector<T>::NRvector(std::vector<T> a) : nn(n), v(n>0 ? new T[n] : NULL)
{
...
template <class T>
NRvector<T> & NRvector<T>::operator=(const NRvector<T> &&rhs) = default; // Move
template <class T>
