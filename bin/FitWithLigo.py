...
def fit_qnm_modes_to_signal(data_dir, offset, num_steps, num_modes=7,
    ...
    signal = subset_Yl2m2[:, 2] - 1j*subset_Yl2m2[:, 1]
    if (include_noise):
        noise = SetupData.ligo_noise_of_subset(data_dir,
                                               offset, num_steps, included_points,
                                               convert=True, bh_mass = 65)
        signal  = signal + noise
    else:
        noise = np.ones((len(included_points)))
    signal_norm = np.sqrt(sum(signal*np.conjugate(signal)))
    # Define residuals (the quantites which when squared and summed give the cost)
    def Mismatch(x, params):

        target = params[0]
        grid = params[2]/x[15]
        target_norm = params[1]
        noise = params[3]
        trial = SetupTrial.construct_complex_trial_from_grid(x, grid)
        mismatch  = 1- abs(sum(trial*np.conjugate(target)/noise)/ \
        np.sqrt(sum(trial*np.conjugate(trial))) / target_norm)
        print("mismatch = ", abs(mismatch))
        return abs(mismatch)*10**-32
    x0 = np.ones(numparams)
    x0[14] = A
    x0[15] = M
    bounds = [(None, None)]*14
    bounds.append((0, .9999))
    bounds.append((.001, np.inf))
    # Actual fitting
    ...
    params = (signal, signal_norm, start_grid, noise)
    X = minimize(lambda x: Mismatch(x, params), x0, bounds=bounds)
    if (plot_waveforms):
        # Plot the Fitted waveform versus the waveform predicted by Numerical
        # Relativity
        plt.plot(start_grid/X['x'][15], signal[:].real, '-g', label = "NR")
